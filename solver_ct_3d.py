...
class Diffusion(object):
    ...
    def sample(self):
        ...
        #         self.ema_weights[name] = {pname: p.data.clone().to('cpu') for pname, p in module.named_parameters()}
    ...
    def simplified_ddnm_plus(self, model):
        ...
        if self.args.init == "random":
            print(f"initializing from random gaussian noise")
            x = torch.randn(
                x_orig.shape[0],
        ...
        elif self.args.init == "y_same_noise":
            print(f"initializing from Ap(y) with same gaussian noise across slices")
            noise = torch.randn(
                1,
                config.data.channels,
                config.data.image_size,
            ...
            noise = noise.repeat(x_orig.shape[0], 1, 1, 1)
            x = at.sqrt() * Apy.cpu() + (1 - at).sqrt() * noise
        elif self.args.init == "y_diff_noise":
            print(f"initializing from Ap(y) with different gaussian noise across slices")
            noise = torch.randn(
                x_orig.shape[0],
                config.data.channels,
                config.data.image_size,
        ...
        for i, j in tqdm.tqdm(times_pair, total=len(times)):
                ...
                if (self.args.end_t <= t <= self.args.start_t) and (t_mod % self.args.adapt_every_k == 0):   
                    ...
                    for _ in range(args.num_steps):
                        ...
                        for _ in range(self.num_accumulation_round):
                            ...
                            samp_idx = random.sample(range(n), self.max_gpu_mc)
                            bcg = ATy[samp_idx, ...].view(self.max_gpu_mc, 1, config.data.image_size, config.data.image_size)
                            y_idx = y[samp_idx, ...].view(self.max_gpu_mc, 1, yh, yw)
                            xt = xs[-1][samp_idx, ...].to(self.device).view(self.max_gpu_mc, 1, config.data.image_size, config.data.image_size)
                            # id-reg related
                            if self.args.in_dist_reg:
                                x0_id = x_orig_id[samp_idx, ...].to(self.device)
                                et_gt = torch.randn_like(x0_id)
                                xt_id = at.sqrt() * x0_id + (1 - at).sqrt() * et_gt
                                et_id = model(xt_id, t)[:, :2]
                                x0_t_id = (xt_id - et_id * (1 - at).sqrt()) / at.sqrt()
                                loss_reg = torch.mean((x0_id - x0_t_id).pow(2))
                                # gradient accumulation for reg denoising loss
                                loss_reg = loss_reg * self.args.reg_gamma
                                loss_reg.backward()

                            # Use the sampled xt for adaptation. Doesn't have to be the same for every iter.
                            et = model(xt, t)[:, :1]
                            x0_t = (xt - et * (1 - at).sqrt()) / at.sqrt()
                            x0_t = CG(Acg, bcg, x0_t, n_inner=1)
            ...
            with torch.no_grad():
                ...
                if self.args.use_diffusionmbir:
                    ...
                    x0_t_hat = CG(Acg, ATy, x0_t, n_inner=5)
